...
const fetchPlayerStats = async () => {
    console.log('[Stats] Starting to fetch player stats...');
    try {
        const [playersResponse, seasonMatches] = await Promise.all([
            window.AppConfig?.fetchAsset
                ? window.AppConfig.fetchAsset('data/players.json')
                : fetch('data/players.json'),
            loadSeasonMatches()
        ]);

        const data = await playersResponse.json();
        console.log('[Stats] Loaded', data.length, 'players from players.json');
        statsPlayers = data;
...
const updateStatsSummary = (players, leagueSummary) => {
    ...
    setSafeText('summaryMatches', totalMatches);
    setSafeText('summaryGoalsFor', leagueSummary?.goalsFor ?? totalGoals);
    setSafeText('summaryGoalsAgainst', leagueSummary?.goalsAgainst ?? totalGoalsAgainst);
    setSafeText('summaryGoalDiff', goalDiff);
};
...
const getLeagueSummary = async () => {
    console.log('[Stats] Getting league summary...');
    try {
        ...
        if (raw) {
            ...
            if (leagueSummary) {
                console.log('[Stats] Using cached league standings');
                return leagueSummary;
            }
        }

        console.log('[Stats] Fetching log.json for league summary...');
        const response = await (window.AppConfig?.fetchAsset
            ? window.AppConfig.fetchAsset('data/log.json')
            : fetch('data/log.json'));
        if (!response.ok) {
            console.warn('[Stats] Failed to fetch log.json:', response.status);
            return null;
        }
        const json = await response.json();
        const summary = parseLeagueStandings(json);
        console.log('[Stats] Parsed league summary:', summary);
        return summary;
    } catch (error) {
        console.warn('[Stats] League summary load failed:', error);
        return null;
    }
};
    ...
    container.innerHTML = topScorers.map(player => {
        ...
        return `
            <div class="top-scorer-card">
                ...
                <div class="top-scorer-meta">${player.position || 'Player'} • ${player.team || 'Tango FC'}</div>
